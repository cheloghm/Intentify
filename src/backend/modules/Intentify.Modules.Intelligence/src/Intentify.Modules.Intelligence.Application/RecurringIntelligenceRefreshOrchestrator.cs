...
public sealed class RecurringIntelligenceRefreshExecutor(RefreshIntelligenceTrendsService service)
    : IRecurringIntelligenceRefreshExecutor
{
    ...
    ILogger<RecurringIntelligenceRefreshOrchestrator> logger)
{
    ...
    public TimeSpan PollInterval =>
        TimeSpan.FromSeconds(Math.Clamp(options.Value.PollIntervalSeconds, 30, 3600));
    ...
    public async Task RunOnceAsync(CancellationToken ct = default)
    {
        ...
        foreach (var profile in profiles.Take(maxProfilesPerCycle))
        {
            ...
            if (profile.RefreshIntervalMinutes is null
                || profile.RefreshIntervalMinutes < MinProfileRefreshIntervalMinutes)
                continue;

            var tenantId = profile.TenantId.ToString();

            var locations = profile.TargetLocations.Count == 0
                ? (IEnumerable<string>)["global"]
            ...
            {
                var timeWindow = string.IsNullOrWhiteSpace(options.Value.TimeWindow)
                    ? "7d"
                    : options.Value.TimeWindow.Trim();
                ...
                try
                {
                    var status = await trendsRepository.GetStatusAsync(
                        tenantId,
                        profile.SiteId,
                    ...
                    // Keyword and AgeRange are null for background refreshes —
                    // the profile's IndustryCategory acts as the primary search term.
