...
public sealed class EngageOrchestrator
{
    ...
    public async Task<OperationResult<ChatSendResult>> HandleAsync(ChatSendCommand command, CancellationToken cancellationToken)
    {
        ...
        if (site is null)
        {
            _logger.LogWarning("Widget key not found");
        ...
        var context = await _contextAnalyzer.AnalyzeAsync(
            session, recentMessages, command.Message, knowledgeSummary, tenantVocab, bot, 
            visitorBundleResult.Value, cancellationToken);
    ...
    // Reused from old handler - unchanged
    ...
    {
        // (Exact same logic as your original ResolveSessionAsync - kept 100% identical)
